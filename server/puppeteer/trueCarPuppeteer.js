...
async function start(make, model, minYear, zip) {
  ...
  await page.goto(
    `https://www.truecar.com/used-cars-for-sale/listings/lexus/price-11000-52000/`
  );

  let Alldata = [];

  //price
  let priceData = await page.$$eval(
    ".heading-3.margin-y-1.font-weight-bold",
    (data) =>
      data.map((singleData) =>
        parseInt(singleData.textContent.slice(1).split(",").join(""))
      )
  );
  ...
  const imageData = await page.$$eval(
    ".img-container.img-container-block",
    (data) => data.map((singleData) => singleData.querySelector("img").src)
  );
  let correctImage = [];
  ...
  //made and model
  const makeData = await page.$$eval(
    ".vehicle-header-make-model.text-truncate",
    (data) => data.map((singleData) => singleData.textContent)
  );
  const makeD = [],
    modelD = [];
  ...
  //url
  const urlData = await page.$$eval(
    ".linkable.card.card-shadow.vehicle-card._1qd1muk",
    (data) => data.map((singleData) => singleData.querySelector("a").href)
  );
